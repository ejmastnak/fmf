...
\textbf{About}
\end{center}
These are my notes are from the \textit{Exercises} (problem-solving) portion of the class \textit{Kvanta Mehanika} (Quantum Mechanics), given to third-year physics students at the Faculty of Math and Physics in Ljubljana, Slovenia. The exercises are led by Asst. Prof. Toma\v{z} Rejec, who has curated the problem sets and leads us through the solutions. I am making the notes freely available in case they might help others taking a similar course, but credit should go to Prof. Rejec for the problem sets. 
...
\subsection{First Vaje}

\subsubsection{Theory}
\begin{itemize}
	\item The basic quantity in quantum mechanics is the wave function. In one dimensions, this is $ \psi(x, t) $. Any measurable quantity can be derived from the wave function. For example probability density
	\begin{equation*}
	...
	\item How to find the wave function for arbitrary time? You must know the stationary wave function at a given time, e.g. $ \psi(x, 0) $. You then find the wave function for all time by solving the \schro equation
	\begin{equation*}
	...
	\begin{equation*}
		\H = \frac{\op{p}^{2}}{2m} + V(x) \eqtext{where} \op{p} = - i\hbar \pdv{}{x}
	\end{equation*}
	
	\item In practice, we usually solve the stationary \schro equation, which reads
	\begin{equation*}
		\op{H}\psi_{n}(x) = E_{n}\psi_{n}(x)
	\end{equation*}
	...
	We then use the solutions $ \psi_{n}(x) $ of the stationary \schro equation to get the time-dependent wave function $ \psi(x, t) $.
	
	The procedure reads: Start with $ \psi(x, 0) $; the time-dependent wave function at a fixed time. Expand $ \psi(x, 0) $ over a basis of the eigenfunctions $ \psi_{n}(x) $.
	\begin{equation*}
	...
	\item Next, on the energy eigenvalues. There typically as many eigenvalues as the dimension of the corresponding Hilbert space containing the particle's wave functions. Often this is infinite! 
	...
	\item Next, on the eigenfunctions. Suppose we have an energy level without degeneracy, i.e. there is one eigenfunction at that energy eigenvalue. Suppose the eigenfunction is given by
	\begin{equation*}
		\op{H}\psi_{n}(x) = E_{n}\psi_{n}(x)
	\end{equation*}
	If $ \psi_{n} $ is a solution, then mathematically $ \lambda \psi_{n} $ also solves the eigenvalue problem for all $ \lambda \in \mathbb{C} $.
	
	But physically, there is a restriction: the probability density must be normalized. So
	\begin{equation*}
	...
	Suppose we have two wave functions:
	\begin{equation*}
	...
	As an example, the infinite potential well from $ 0 $ to $ a $. The energy eigenvalues are
	\begin{equation*}
...
Find the energy eigenvalues of the bound states of a particle with energy $ E $ in a finite potential well of depth $ V_{0} $ and width $ a $. Assume $ \abs{E} < V_{0} $; note that bound states have negative energies.
\begin{itemize}
...
If two eigenfunctions $ \psi_{1} $ and $ \psi_{2} $ have the same eigenvalue $ E $, than any linear combination of $ \psi_{1} $ and $ \psi_{2} $ is also an eigenfunction with eigenvalue $ E $. In equation form, if $ \op{H}\psi_{1}(x) = E \psi_{1}(x) $ and $ \op{H}\psi_{2}(x) = E \psi_{2}(x) $ then
\begin{equation*}
	\op{H}\left(\alpha \psi_{1}(x) + \beta \psi_{2}(x)\right) = E \left(\alpha \psi_{1}(x) + \beta \psi_{2}(x)\right)
\end{equation*}
...
\begin{itemize}
	...
	\begin{equation*}
		\op{H}\psi(-x) = E\psi(-x)
	\end{equation*}
	This means that if $ \psi(x) $ is an eigenfunction of $ \op{H} $ for the eigenvalue $ E $, then $ \psi(-x) $ is also an eigenfunction for the same eigenvalue.
...
\subsection{Second Vaje}

\subsubsection{Theory}
Even bound states in a finite potential well of width $ a $ and depth $ V_{0} $ obey
\begin{equation*}
...
\begin{itemize}
	...
	\begin{equation*}
		\op{H} \psi = E \psi \eqtext{where} \op{H} = -\frac{\hbar^{2}}{m}\pdv[2]{}{x} + V(x)
	\end{equation*}
...
\subsection{Vaje 3: Formalism of Quantum Mechanics} 

\subsubsection{Theory}
\begin{itemize}
	\item So a derivation of uncertainty principle stuff for two arbitrary observables, e.g $ A $ and $ B $. What is the uncertainty in the product $ \Delta A \Delta B $?
	
	We start by assigning both $ A $ and $ B $ a corresponding Hermitian operator (Hermitian because $ A $ and $ B $ are observable). I will drop the operator notation. Hopefully its clear from context. The operators are
	\begin{equation*}
...
\subsubsection{Wave Functions and Uncertainty Principle}
\textit{Find all wave functions for which $ \Delta x \Delta p = \frac{\hbar}{2} $}.
\begin{itemize}
	\item To solve this, both inequalities in the above proof must be equalities. First, the Cauchy-Schwartz inequality is an equality for two linearly dependent vectors.
	\begin{equation*}
...
Given a Gaussian wave function$ \psi(x, 0) $ and the time-independent Hamiltonian $ H = \frac{p^{2}}{2m} + V(x) $, find the time evolution $ \psi(x, t) $.

\textbf{Theory: Time Evolution of a Wave Function}
\begin{itemize}
...
\begin{itemize}
	\item \textbf{TODO: record these!} First, a distributive property
	\begin{equation*}
		[AB, C] = A[B, C] + [A, C]B
	\end{equation*}
	and second:
...
\begin{itemize}
	\item At the last exercise set, we wanted to find the uncertainties
	\begin{equation*}
		\Delta x(t) = \sqrt{\ev{x^{2}, t} - \ev{x, t}^{2}} \eqtext{and} \Delta p(t) = \sqrt{\ev{p^{2}, t} - \ev{p, t}^{2}}
	\end{equation*}
...
\textbf{Theory: Theorem Ehrenfest Theorem}\\
...
\end{equation*}
Compute the wavefunction's time evolution $ \ket{\psi, t} $ the time-dependent expected values $ \ev{x, t} $ and $ \ev{p, t} $ and use this to determine the validity of the Ehrenfest theorem. Finally, determine the product of uncertainties
\begin{equation*}
	\Delta x(t) \Delta p(t)
\end{equation*}
\begin{itemize}
...
\textbf{Theory: Coherent States of the Quantum Harmonic Oscillator}
\begin{itemize}
