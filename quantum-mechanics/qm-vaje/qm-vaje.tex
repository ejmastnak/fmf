...
\subsection{Thirteenth Exercise Set}

\subsubsection{Theory: Degenerate Perturbation Theory}
\begin{itemize}
	\item Consider $ H = H_{0} + H' $. And eigenvalue relation
	\begin{equation*}
		H_{0} \ket{\psi}  = E \ket{\psi}
	\end{equation*}
	where the energy $ E $ is $ N $-times degenerate, meaning
	\begin{equation*}
		\ket{\psi} = \sum_{i}^{N}c_{i}\ket{i}
	\end{equation*}
	where for each $ \ket{i} $
	\begin{equation*}
		H\ket{i} = E \ket{i}
	\end{equation*}
	
	\item We consider only first-order correction.  Procedure:
	\begin{enumerate}
		\item Create a matrix of perturbation elements in a chosen basis of the subspace of degenerate states, e.g. in our case $ \ket{i} $. 
		\begin{equation*}
			\begin{pmatrix}
				\threebraket{1}{H'}{1} & \threebraket{1}{H'}{2} &  \cdots & \threebraket{1}{H'}{N} \\
				\threebraket{2}{H'}{1} & \threebraket{2}{H'}{2} &  \cdots & \threebraket{2}{H'}{N} \\
				\vdots & \cdots & \ddots & \vdots\\
				\threebraket{N}{H'}{1} & \threebraket{N}{H'}{2} &  \cdots & \threebraket{N}{H'}{N} 
			\end{pmatrix}
		\end{equation*}
		
		\item Diagonalize the matrix
		
		\item The corrected energies are then
		\begin{equation*}
			\t{E}_{j} = E + \lambda_{j} 
		\end{equation*}
		In general the degeneracy is broken if the $ \lambda_{j} $ are different. 
		
		Note that if certain eigenvalues $ \lambda_{j} $ are the same, then at least some degeneracy remains in the perturbed energy eigenvalues.
		
		\item Then the new eigenstates are 
		\begin{equation*}
			\ket{\p_{j}} = \sum_{k=1}^{N}u_{j, k} \ket{k}
		\end{equation*}
		where $ u_{j} $ is the perturbation matrix's eigenvector corresponding to $ \lambda_{j} $. 
	\end{enumerate}
\end{itemize}

\subsubsection{Hydrogen Atom in a Homogeneous Electric Field (rough draft)}
...
\begin{itemize}
	...
	\begin{align*}
		&\ket{200}\ket{\ua}, \qquad \ket{200}\ket{\da}\\
		&\ket{211}\ket{\ua}, \qquad \ket{211}\ket{\da}\\
		&\ket{210}\ket{\ua}, \qquad \ket{210}\ket{\da}\\
		&\ket{21-1}\ket{\ua}, \qquad \ket{21-1}\ket{\da}\\
	\end{align*}
	
	\item  Next we consider the radially-dependent wavefunction:
	\begin{equation*}
	...
	\item On to our perturbation matrix, which is $ 8 \cross 8 $ because of $ N = 8 $ degeneracy. A general matrix element takes the form
	\begin{equation*}
		\bra{m_{s}} \threebraket{2lm}{H'}{2l'm'} \ket{m_{s}'}
	\end{equation*}
	...
	\begin{equation*}
		\threebraket{2lm}{H'}{2l'm'}\braket{m_{s}}{m_{s}'} = \threebraket{2lm}{H'}{2l'm'} \delta_{m_{s}, m_{s}'}
	\end{equation*}
	...
	\begin{equation*}
		\threebraket{2lm}{[H', L_{z}]}{2l'm'} = \threebraket{2lm}{(H' L_{z} - L_{z}H')}{2l'm'}
	\end{equation*}
	We distribute to get two matrix elements
	\begin{equation*}
		\threebraket{2lm}{H'L_{z}}{2l'm'} - \threebraket{2lm}{L_{z}H'}{2l'm'}
	\end{equation*}
	...
	\begin{align*}
		& \threebraket{2lm}{H'L_{z}}{2l'm'} - \threebraket{2lm}{L_{z}H'}{2l'm'} = hm'\threebraket{2lm}{H'}{2l'm'} - \hbar m \threebraket{2lm}{H'}{2l'm'}
	\end{align*}
	Take a closer look: (use $ L_{z}^{\dagger} = L_{z} $)
	\begin{align*}
		&\threebraket{2lm}{L_{z}H'}{2l'm'} \\
		& = \threebraket{L_{z}2lm}{H'}{2l'm'} \\
		&= \threebraket{(\hbar m)2lm}{H'}{2l'm'}\\
		& = (\hbar m)^{*}\threebraket{2lm}{H'}{2l'm'}\\
		& = \hbar m \threebraket{2lm}{H'}{2l'm'}
	\end{align*}
	
	\item Back to the main thing
	\begin{align*}
		 & hm'\threebraket{2lm}{H'}{2l'm'} - \hbar m \threebraket{2lm}{H'}{2l'm'}\\
		 & = \hbar(m' - m)\threebraket{2lm}{H'}{2l'm'} = 0
	\end{align*}
	We we've used $ [H', L_{z}] = 0 $ to get the zero. This implies 
	\begin{equation*}
		m' \neq m \implies \threebraket{2lm}{H'}{2l'm'} = 0
	\end{equation*}
...
\begin{itemize}
	...
	\begin{align*}
		& \threebraket{2lm}{\{H', P\}}{2l'm'} =\\
		 &\threebraket{2lm}{H' P}{2l'm'} + \threebraket{2lm}{PH'}{2l'm'}\\
		& = \threebraket{2lm}{H'}{2l'm'} (-1)^{l'} + \threebraket{2lm}{H'}{2l'm'}(-1)^{l}\\
		& = \big[(-1)^{l'} + (-1)^{l}\big]\threebraket{2lm}{H'}{2l'm'} = 0
	\end{align*}
	We end up with
	\begin{equation*}
		(-1)^{l'} + (-1)^{l} \neq 0 \implies \threebraket{2lm}{H'}{2l'm'} = 0
	\end{equation*}
	Or if $ l $ and $ l' $ are both even or both odd then $ \threebraket{2lm}{H'}{2l'm'} = 0 $. \textbf{TODO} review if it isn't switched. 
...
\begin{itemize}
	...
	\item The $ \threebraket{2lm}{H'}{2l'm'} = 0  $ from above means that the diagonal matrix elements in the diagonal blocks are zero. 
	...
	\begin{equation*}
		\begin{pmatrix}
		0 & \threebraket{200}{H'}{210}\\
		\threebraket{210}{H'}{200} & 0
		\end{pmatrix}
	\end{equation*}
	The matrix elements of a Hermitian operator $ H $ form a hermitian matrix, which means
	\begin{equation*}
		\threebraket{200}{H'}{210} = \threebraket{210}{H'}{200}^{*}
	\end{equation*}
	
	\item We have only one non-zero matrix element from originally 64.
	\begin{align*}
		\threebraket{200}{H'}{210} &= \int \diff^{3}\vec{r} \psi_{200}^{*}(\r) (-q \E z)\psi_{210}(\r)\\
	...
	\begin{align*}
		\threebraket{200}{H'}{210} &= \int \diff^{3}\vec{r} \psi_{200}^{*}(\r) (-q \E z)\psi_{210}(\r)	(-q\E)\frac{2}{\sqrt{3}}\frac{\sqrt{3}}{4\pi} \cdot \frac{r_{B}}{8} \int_{0}^{\infty}u^{4}\diff u\left(1 - \frac{u}{2}\right)e^{-u} \cdot \left(\frac{2}{3}\right) \cdot \left(2\pi\right)\\
	\end{align*}
	We find the $ u $ integral with the gamma function. Result is
	\begin{align*}
			\threebraket{200}{H'}{210} & = -q\E r_{B}\frac{1}{12}\left(4! - \frac{1}{2}5!\right) = 3q\E r_{B}
	\end{align*}
