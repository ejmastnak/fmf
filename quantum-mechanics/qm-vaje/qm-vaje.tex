...
\begin{itemize}
	\item Review from the last exercise set: we had just begun the problem, and established the two possible bases to describe the problem---the product basis and the good quantum number basis.
	...
	\item The second basis, called the good quantum number basis, uses the commutativity of $ S_{2} $, $ S_{z} $, $ S_{1}^{2} $ and $ S_{2}^{2} $. Recall total spin is the vector sum $ \vec{S} = \vec{S}_{1} + \vec{S}_{2} $.  We write the states in this basis as
	\begin{equation*}
...
\begin{itemize}
	...
	\item Having discussed the single bound state for $ S = 3/2 $, we now consider the free scattering states. Recall that for a particle incident on the origin from the left (i.e. from the the region $ x < 0 $), the reflected wavefunction takes the general form:
	\begin{equation*}
		\ket{\psi_{r}} = \left(e^{ik x} + re^{-i\kappa x}\right)\ket{SM},
	\end{equation*}
	while the transmitted wavefunction takes the general form
	\begin{equation*}
		\ket{\psi_{\text{t}}} = t e^{ikx}\ket{SM}
	\end{equation*}
	where $ r $ and $ t $ parameterize the scattering matrix.
	...
	\begin{equation*}
		...
		\end{bmatrix}
	\end{equation*}
	where we have introduced a subscript $ S $ to make the dependence of $ r $ and $ t $ on the value of the total spin $ S $ explicit. The possible values of $ \kappa_{S} $ for $ S \in \{1/2, 3/2\} $ are
	\begin{equation*}
	...
	\begin{align*}
		&\ket{\psi^{(1/2)}_{\text{r}}} = \left(e^{ik_{1/2} x} + r_{1/2}e^{-i\kappa x}\right)\ket{\tfrac{1}{2}M} \quad \ket{\psi^{(1/2)}_{\text{t}}} = t_{1/2} e^{ik_{1/2}x}\ket{\tfrac{1}{2}M}\\
		&\ket{\psi^{(3/2)}_{\text{r}}} = \left(e^{ik_{3/2} x} + r_{3/2}e^{-i\kappa x}\right)\ket{\tfrac{3}{2}M} \quad \ket{\psi^{(3/2)}_{\text{t}}} = t_{3/2} e^{ik_{3/2}x}\ket{\tfrac{3}{2}M}
	\end{align*}
	form a complete basis of the momentum Hilbert space of free scattering states. 
	
	\item Just to review the problem: particle 1, in state $ \ket{\ua} $, is incident from the left on particle 2, which is fixed at the origin and in state $ \ket{0} $, and we are interested in the scattered spin states of the two particles.
	
	The incident part of the wavefunction takes the form
	\begin{equation*}
		\ket{\psi_{\text{in}}} = e^{ikx} \ket{\ua} \ket{0}
	\end{equation*}
	Note that this wavefunction is written in the product basis---it is the product of the first particle's spin state $ \ket{\ua} $ and the second particle's spin state $ \ket{0} $. However, the above eigenfunctions $ \ket{\psi^{(S)}_{\text{r}}} $ and $ \ket{\psi^{(S)}_{\text{t}}} $ spanning the Hilbert space of free scattering are written in the good quantum number basis, since they involve $ \ket{SM} $. 
	
	Evidently, we need a way to transform between the product basis and the good quantum number basis---we make the transformation using the Clebsch-Gordan (CG) coefficients.
\end{itemize}	

\begin{figure}[hbtb!]
\centering
\includegraphics[width=\linewidth]{cg-coefficients}
