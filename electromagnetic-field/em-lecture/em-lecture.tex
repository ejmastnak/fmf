...
\begin{itemize}
    \item We begin with the funamental relationship of electrostatics: Coulomb's law. In equation form, the electrostatic force $ \vec{F} $ felt by a point charge $ q_{1} $ at position $ \r_{1} $ in the presence of a second point charge $ q_{2} $ at position $ \vec{r}_{2} $ is
    \begin{equation*}
...
\begin{itemize}
    ...
    \item For the entirety of this course, we will analyze the interaction of charged particles using the concept of an \textit{electric field}. To speak of an electric field, we must consider at least two charges: one charge creates the electric field, which is then experienced by the second charge. In terms of electric field, the Coulomb electrostatic force $ \vec{F}_{2} $ experienced by particle 2 because of the field created by particle $ 1 $ is
    \begin{equation*}
        \vec{F}_{2} = q_{2} \E_{1}.
    \end{equation*}
