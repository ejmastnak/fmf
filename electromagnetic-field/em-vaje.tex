...
\begin{itemize}
	...
	\begin{equation*}
        H_{r} = r\pdv{H_{z}}{r} \eqtext{and} H_{r}\big |_{\partial} = 0 \implies a\pdv{H_{z}}{r} (a, \phi) = 0
	\end{equation*}
...
\begin{itemize}
    \item In TE mode, the relavant boundary condition involves the perpendicular magnetic field $ H_{\perp} $. Along the circular surface, the perpendicular magnetic field is $ H_{r} = r\pdv{H_{z}}{r} $. Requiring $ H_{\perp} = 0 $ leads to the condition $ m = 0, 2, 4, 6, \ldots $, as in TE mode.
	
    \item Along the bottom flat surface at $ \phi = 0 $, the perpendicular component of magnetic field is
	\begin{equation*}
		H_{\perp} = \pdv{H_{z}}{\phi} (\phi = 0, r) 
	\end{equation*} 
    and the boundary condition $ H_{\perp} = 0 $ requires the angular solutions are cosine terms. Along the vertical flat surface at $ \phi = \pi/2 $, the boundary condition reads
	\begin{equation*}
		H_{\perp} = \pdv{H_{z}}{\phi}\left(\frac{\phi}{2}, r\right) = 0
	\end{equation*}
    as for the bottom flat surface, this condition requires the angular solutions are cosine terms.
	
    In TE mode, the case $ m = 0 $ is valid because the angular cosine solutions are constant and thus non-trivial, at $ m = 0 $. Similarly, the radial Bessel functions are nontrivial for $ m = 0 $, and we get a valid solution. In TE mode for a quarter-circle cross section, the valid values of $ m $ are thus $ m = 0, 2, 4, \ldots $.
