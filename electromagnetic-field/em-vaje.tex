...
\begin{itemize}
	...
	\begin{equation*}
		\A = \frac{\mu_{0}}{4 \pi} \int \frac{\vec{j}(\tvec{r})\diff^{3}\tilde{r}}{\abs{\r - \tvec{r}}}
	\end{equation*}
...
\textit{A conducting loop with radius $ a $ carries a current $ I $. What is the magnetic vector potential far from the conducting loop?}
\begin{itemize}
	...
	Assume the the loop lies in the $ x, y $ plane with the $ z $ axis normal to the loop. For mathematical convenience, we rotate the $ x, y $ plane so $ \r $ lies in the $ x, z $ plane; this just gives us one less non-zero component to work with, since the $ y $ component of $ \r $ is zero. In polar coordinates $ \r $ reads
	\begin{equation*}
	...
	\item We can now but the pieces together in the equation for $ \A $. First,
	\begin{align*}
	...
	\begin{equation*}
		A_{y}(r) = \frac{\mu_{0}I}{4\pi} \frac{a}{r} \int_{0}^{2\pi} \cos^{2} \tilde{\phi}
		 \frac{2a}{r}\sin \theta \diff \tilde{\phi} = \frac{\mu_{0}I}{4\pi} \frac{a^{2}}{r^{2}}  \sin \theta 
	\end{equation*}
	or, in vector form,
	\begin{equation*}
		\A(\r) = \frac{\mu_{0}I}{4\pi} \frac{a^{2}}{r^{2}}  \sin \theta \uvec{y} = \frac{\mu_{0}I}{4\pi} \frac{a^{2}}{r^{2}} \uvec{z} \cross \uvec{r}
	\end{equation*}
	The second expression is preferable: the first, in terms of $ \uvec{y} $, holds only with $ x, y $ plane is rotated so $ \r $ lies in the $ x, z $ plane. The second, which uses $ \sin \theta \uvec{y} = \uvec{z} \cross \uvec{r} $, holds for any orientation of the $ x, y $ plane. 
	...
	\begin{equation*}
		\A(\r) = \frac{\mu_{0} \m \cross \r}{4\pi r^{3}}
	\end{equation*}
...
\textit{Consider a charged, insulating disk with uniform surface charge density $ \sigma $ and radius $ a $. The disk rotates uniformly about an axis through its center with constant angular velocity $ \vec{\omega} = (0, 0, \omega) $. Find the magnetic field along the axis of rotation.}
\begin{itemize}
	\item We choose a coordinate system so the disk lies in the $ x, y $ plane and the axis rotation coincides with the $ z $ axis. We start with the general Biot-Savart law
	\begin{equation*}
		\B (\r) = \frac{\mu_{0}}{4\pi}\int \frac{\vec{j}(\tvec{r}) \cross (\r - \tvec{r})}{\abs{\r - \tvec{r}}^{3}} \diff^{3}\tilde{r}
	\end{equation*}
